...
class EvaluationJob(ImagingRecordBase):
    ...
    __table_args__ = (
        ...
        Index("ix_evaluation_job_retry", "status", "next_retry_at", "created_at"),
    )
    ...
    status: Mapped[str] = mapped_column(String(32), nullable=False, default="queued", server_default=text("'queued'"), comment="VARCHAR(32): queued/running/retry_wait/completed/failed/cancelled/dead_letter")
    ...
    lease_owner_id: Mapped[str | None] = mapped_column(String(128), nullable=True, comment="VARCHAR(128)|NULL: Evaluation Worker lease owner")
    lease_generation: Mapped[int] = mapped_column(BigInteger, nullable=False, default=0, server_default=text("0"), comment="BIGINT: Evaluation Worker lease 世代，防止旧 Worker 回写")
    lease_expires_at: Mapped[datetime | None] = mapped_column(DATETIME(fsp=6), nullable=True, comment="DATETIME(6)|NULL: Evaluation Worker lease 到期时间，UTC")
    heartbeat_at: Mapped[datetime | None] = mapped_column(DATETIME(fsp=6), nullable=True, comment="DATETIME(6)|NULL: Evaluation Worker 最近心跳时间，UTC")
    retry_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default=text("0"), comment="INT: Evaluation Job 已安排的重试次数")
    next_retry_at: Mapped[datetime | None] = mapped_column(DATETIME(fsp=6), nullable=True, comment="DATETIME(6)|NULL: 下一次可执行时间，UTC")
    result_artifact_id: Mapped[str | None] = mapped_column(String(64), nullable=True, comment="VARCHAR(64)|NULL: metric_summary Artifact ID")
    started_at: Mapped[datetime | None] = mapped_column(DATETIME(fsp=6), nullable=True, comment="DATETIME(6)|NULL: 首次开始时间，UTC")
    finished_at: Mapped[datetime | None] = mapped_column(DATETIME(fsp=6), nullable=True, comment="DATETIME(6)|NULL: 终态完成时间，UTC")
...
class EvaluationRun(ImagingRecordBase):
    ...
    scorer_fingerprint: Mapped[str] = mapped_column(String(64), nullable=False, comment="CHAR(64): scorer 指纹")
    experiment_fingerprint: Mapped[str] = mapped_column(String(64), nullable=False, comment="CHAR(64): experiment 指纹")
    case_split_sha256: Mapped[str] = mapped_column(String(64), nullable=False, comment="CHAR(64): 冻结 case split 合同摘要")
    denominator_contract_sha256: Mapped[str] = mapped_column(String(64), nullable=False, comment="CHAR(64): 冻结双分母合同摘要")
    input_manifest_artifact_sha256: Mapped[str] = mapped_column(String(64), nullable=False, comment="CHAR(64): 输入 manifest Artifact 摘要")
    sanitization_artifact_sha256: Mapped[str] = mapped_column(String(64), nullable=False, comment="CHAR(64): 脱敏证明 Artifact 摘要")
    status: Mapped[str] = mapped_column(String(32), nullable=False, default="started", server_default=text("'started'"), comment="VARCHAR(32): started/succeeded/failed/cancelled/unknown")
    ...
    error_code: Mapped[str | None] = mapped_column(String(80), nullable=True, comment="VARCHAR(80)|NULL: 稳定错误码")
    error_message: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="VARCHAR(500)|NULL: 脱敏错误摘要")
    started_at: Mapped[datetime] = mapped_column(DATETIME(fsp=6), nullable=False, comment="DATETIME(6): Run 开始时间，UTC")
    finished_at: Mapped[datetime | None] = mapped_column(DATETIME(fsp=6), nullable=True, comment="DATETIME(6)|NULL: Run 完成时间，UTC")


class EvaluationArtifact(ImagingRecordBase):
    __tablename__ = "evaluation_artifact_record"
    __table_args__ = (
        UniqueConstraint("job_id", "run_id", "artifact_kind", name="uq_evaluation_artifact_run_kind"),
    ...
    artifact_kind: Mapped[str] = mapped_column(String(64), nullable=False, comment="VARCHAR(64): input_manifest/sanitization/case_result/failure_summary/metric_summary/paired_ab_summary")
