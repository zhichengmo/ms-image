...
from apps.backend.core.exception import CustomException
...
class DalBase:
    ...
    async def get_data(
        ...
        if not isinstance(v_start_sql, SelectType):
            v_start_sql = select(self.model).where()
        ...
        if data and v_schema:
            return await self.out_dict(data, v_schema=v_schema)
    ...
    async def out_dict(
        ...
        if v_schema:
            # 先创建模型实例
            model_instance = v_schema.model_validate(obj)
            # 等待所有异步验证器完成
            if hasattr(model_instance, 'model_post_init_async'):
                # 优先使用异步方法
                try:
                    # 检查参数数量
                    import inspect
                    sig = inspect.signature(model_instance.model_post_init_async)
                    if len(sig.parameters) > 1:  # self + 其他参数
                        # 有额外参数，跳过调用
                        pass
                    else:
                        # 只有self参数，可以安全调用
                        await model_instance.model_post_init_async()
                except (TypeError, AttributeError):
                    # 如果调用失败，忽略错误
                    pass
            elif hasattr(model_instance, 'model_post_init'):
                # 如果没有异步方法但有model_post_init，尝试调用
                try:
                    # 检查参数数量
                    import inspect
                    sig = inspect.signature(model_instance.model_post_init)
                    if len(sig.parameters) > 1:  # self + 其他参数
                        # 有额外参数，跳过调用
                        pass
                    else:
                        # 只有self参数，可以安全调用
                        model_instance.model_post_init()
                except (TypeError, AttributeError):
                    # 如果调用失败，忽略错误
                    pass
            return model_instance.model_dump()
        if self.schema:
            model_instance = self.schema.model_validate(obj)
            if hasattr(model_instance, 'model_post_init_async'):
                # 优先使用异步方法
                await model_instance.model_post_init_async()
            elif hasattr(model_instance, 'model_post_init'):
