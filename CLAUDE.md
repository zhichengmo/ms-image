...
本仓库是宠物多模态影像 AI（人工智能）服务，不是 MS-Scaffold（通用微服务脚手架）。当前代码仍是 XRay validation-only（X 光仅验证）骨架，目标架构处于 `DESIGNED_NOT_IMPLEMENTED`（已设计尚未实现）状态。
...
- 实体 DAL 继承 `app.core.crud.DalBase`；API、Service、Worker 和脚本不得直接拼 SQLAlchemy 查询。
...
- 当前 XRay 专项表、API、Worker 和 Stub Provider（桩 AI 提供方）属于 validation-only 骨架，不是目标通用架构。
- 目标在线 10 表和隔离评测 4 表尚未实现；10+4 是候选基线，不是数量上限。
- 目标 8 个在线业务 Service 和 5 个 Stage Service 尚未完整实现。
