...
- 当前状态：`p0_secret_gate_passed / asset_baseline_checkpoint`（P0 密钥门禁已通过/正在建立资产基线检查点）
- 当前分支：`codex/ms-image-refactor`；所有后续工作按业务 owner 垂直切片提交，每次验证后立即推送并核对远端 SHA，推送失败不得进入下一切片。
- 下一步：完成资产基线 checkpoint 推送，再核对 P0 工程阻断；无硬阻断时实施 P1A Session -> Study+Series -> Image，不先接医学 Provider。
- 活动入口：
