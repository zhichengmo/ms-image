...
状态：`CURRENT_PLAN / P1_CODE_IMPLEMENTED / MIGRATION_NOT_AUTHORIZED`（当前计划/P1 代码已实现/未授权执行迁移）
...
| 范围 | 当前已确认 | 目标 | 处理 |
|---|---|---|---|
| ORM | P1 已有 Session/Study/Series/Image 与现有 Outbox 代码；旧 5 张 AI 配置表 + 10 张 `xray_accuracy_*` 表仍在 | 当前基线为在线 10 表 + 评测 4 表；必要时按准入证据增表 | P1 已语义重建；其余不逐表照搬 |
| 业务根 | P1 已有 Session/Study/Series/Image；旧 XRaySession/XRayRun 仍作兼容来源 | Session/Study/Task，XRay 仅为模态 | P1 已完成；Task 待 P2 |
...
| 异步 | Image Outbox/Relay/Worker/reconcile/revision 代码已完成；Task/Stage/Call 尚未实现 | 通用 Image/Stage/Call 恢复链 | P1 已闭环代码；P2 复用语义 |
...
P1C 不是 P2 的提前实现，而是 Image 生命周期闭环所需的最小可靠执行边界。当前代码已经完成 P1A/P1B/P1C，并在 `6dfd8d1`（2026-08-19，`chore: close P1 imaging implementation`）完成 P1 实现收口：
`Image uploading -> validating` 与 `outbox_record(validate_image)` 同事务；OSS HEAD、流式 hash、真实格式
校验在事务外；通过后才 ready，失败进入 quarantined。P1 的当前状态只可标记 `CODE_IMPLEMENTED / NOT_MIGRATED / NOT_RUNTIME_VALIDATED`，不能写成真实环境通过。
...
| 公共用户/宠物/病历 | 不迁公共主数据 | 只保存外部业务 opaque ID（不透明标识）；旧 `vet-platform` 导入时写历史来源引用 |
...
回滚只改变 `ms-image` `ControlPlane`（控制面）的 release routing（发布路由）或暂停候选 Profile（流程配置），不删除或重解释历史 Task/Stage/Call/Report。旧 V2 fallback（降级）结果只能作为旧系统迁移追溯，不能复制进新库。出现新增异常漏诊、正常误报/不确定超门槛、实际模型漂移、队列 SLA 失守或数据污染时立即 No-Go（禁止放行）。
